...
files <- list.files(doc_folder, "*.txt", full.names = TRUE)
model <- topicdict_model(files, dict, stopwords = stops)
```
And run three models: with weak, medium, and heavy confidence in the seed words

Low confidence: mean = 0.1, sd = 0.09
```{r}
model <- topicdict_model(files, dict, 
                         stopwords = stops, gamma_1 = 1,gamma_2 = 9)
...
Medium confidence: mean = 0.5, sd = 0.15
```{r}
model <- topicdict_model(files, dict, 
                         stopwords = stops, gamma_1 = 5,gamma_2 = 5)
model <- topicdict_train(model, iter = 60)
post <- posterior(model)
tt <- top_terms(post, n = 100, show_seed = TRUE) 
tt[1:20,]
...
High confidence: mean = 0.9, sd = 0.09
```{r}
model <- topicdict_model(files, dict, 
                         stopwords = stops, gamma_1 = 9, gamma_2 = 1)
model <- topicdict_train(model, iter = 60)
post <- posterior(model)
tt <- top_terms(post, n = 100, show_seed = TRUE) 
tt[1:20,]
...
Medium confidence but not so sure: mean = 0.5, sd = 0.2
```{r}
model <- topicdict_model(files, dict, 
                         stopwords = stops, gamma_1 = 2.5, gamma_2 = 2.5)
model <- topicdict_train(model, iter = 60)
post <- posterior(model)
tt <- top_terms(post, n = 100, show_seed = TRUE) 
tt[1:20,]
