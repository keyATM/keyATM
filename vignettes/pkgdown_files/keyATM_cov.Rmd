...
table(bills_cov) # this is a binary indicator
```

Please make sure that **the order of covariates is the same as the order of documents**.
The easiest way to match the order and the dimension of covariats with the documents is to use
the [`docvars`](https://quanteda.io/reference/corpus.html) argument in **quanteda** when you
construct a corpus object.
```{r, eval=FALSE, message=FALSE, fig.align='center'}
CORPUS <- corpus(raw_docs, text_field = "text", docvars = COVARIATES)
[...preprocessing and make a dfm object...]
covariates <- docvars(DFM_OBJECT)
```
