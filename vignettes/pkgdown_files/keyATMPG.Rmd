...
\begin{align}
  ...
  &= \prod_{k=1}^{K-1} \binom{N_{dk}}{n_{dk}} 
  \frac{\exp(\psi_{dk})^{n_{dk}}}{(1 + \exp(\psi_{dk}))^{N_{dk}}} \\ \label{eq:sb-multi}
...
%
\begin{align}
  \text{vec}({\blambda}) &\sim \cN(\text{vec}(\bmu_0), \bSigma_0 \otimes \bLambda_0^{-1}) \\ \label{eq:prior_lambda}
...
% $\bLambda_\lambda = \frac{1}{\sigma_\lambda^2} \mathbf{1}_{M}$.
$\bSigma_0 \otimes \bLambda_0^{-1}$ becomes a diagonal matrix and 
equation~\eqref{eq:prior_lambda} is the same
...
## Estimation
We sample $\blambda$, $\bPsi$, and P{\'{o}}lya-gamma auxiliary variables $\bomega$.


### Sampling $\bPsi$
Equation~\eqref{eq:sb-multi} has the same form as Theorem 1 of Polson et al. (2013)
