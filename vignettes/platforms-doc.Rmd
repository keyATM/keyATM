...
Binary features give rather similar results:
```{r}
dtm_over <- dfm_select(dtm_over, pattern = unlist(new_dict), selection = "keep", valuetype = "fixed")
bina <- dfm_tfidf(dtm_over, scheme_tf = "boolean", scheme_df = "unary")
dtm_bina <- dfm_lookup(bina, new_dict)
res <- data.frame(dtm_bina)
bina_res <- data.frame(docvars(dtm_bina),
                       POS = (res$Con - res$Pro) / (res$Con + res$Pro),
                       logitPOS = log(res$Con) - log(res$Pro))
bina_subset <- subset(bina_res, party %in% partyabbrev)
ggplot(bina_subset, aes(date, logitPOS, color = party)) +
  geom_line() +
  geom_point() +
  geom_vline(xintercept = c(1992, 1997), alpha = 0.1, size = 5 ) +
  scale_color_manual(values = c("blue", "red", "orange", "orange", "orange")) +
