...
diagnosis_topic_recovery_heatmap <- function(post, n=25, topicvec=c()){
  ...
  topwords %>%
    ...
    num <- length(unique(res$Estimated))
		if(is.null(topicvec)){
			...
			message("Topicvec length does not match the topic number")
			topicvec <- 1:num
		}
...
name <- "Sim1"
create_sim_data_from_fitted(saveDir=paste0(data_folder, name, "/"), alpha_vec, phiR, phiS, p_original_vec, D=200, lambda=200)
```
...
seed_list <- list(c("w33t5 w65t5 w209t5"),
                 c("w223t2 w536t2 w541t2"),
								 c("w651t6 w45t6 w50t6"),
								 c("w6t3 w0t3 w26t3"))
model <- create_model("Sim1", seed_list=seed_list, extra_k=4)
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
diagnosis_topic_recovery_heatmap(post, 10, topicvec=c(5,2,6,3,1,4,7,8))
```
