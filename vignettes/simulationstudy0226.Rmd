...
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
explore_$show_words(c('w65t5','w209t5', 'w223t2', "w536t2", "w1656t3",
											"w541t2", "w6t3", "w2629t3", "w1668t3"))
```

```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
seed_list <- list(c("w65t5 w209t5"),
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
get_lda_result("Sim1", iter=iter_num, k=11, topicvec=c(8,10,2,5,9,3,1,4,11,6,7))
```
...
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(4,7,11,15,18,2,3,10,13,17,1,6,16,5,8,14,9,12)
```
...
get_lda_result("Sim1", iter=iter_num, k=18, 
							 topicvec=c(9,12,14,16,4,5,18,8,15,13,1,2,6,10,11,7,17,3))
```
...
### Heatmap
If the number of topics is fewer than the truth, there are contaminated topics. Topic 4 and 5 are regular topic.
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
diagnosis_topic_recovery_heatmap(post, 25, topicvec=new_order)
```
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
get_lda_result("Sim1", iter=iter_num, k=10, topicvec=c(7,8,10,6,9,2,1,4,5,3))
```
...
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(5,6,1,2,9,10,7,4,3,8)
```
...
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(5,7,9,10,1,8,2,4,3,6)
```
...
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(1,4,2,3,6,5,10,7,8,9)
```
