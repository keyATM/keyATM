...
visualize_keywords <- function(docs, keywords, prune = TRUE, label_size = 3.2) {
  ...
  # Check keywords
  keywords <- check_keywords(unique(unlisted), keywords, prune)$keywords
...
check_keywords <- function(
  ...
  all_pruned_topic_idx <- which(as.vector(num_keywords) == 0)

  if (stop_if_all_pruned && length(all_pruned_topic_idx) != 0) {
    zero_names <- names(keywords)[all_pruned_topic_idx]
  ...
  return(list(keywords = keywords, all_pruned_topic_idx = all_pruned_topic_idx))
}
...
#' @param keywords a list of keywords
#' @param docs a keyATM_docs object, generated by \code{keyATM_read()} function
#' @param prune logical, whether to prune keywords that do not appear in the documents
#' @export
...
#' }
refine_keywords <- function(keywords, docs, prune = TRUE) {
  ...
  checked <- check_keywords(
    unique_words,
    keywords,
    prune = prune,
    stop_if_all_pruned = FALSE
  )
  keywords_checked <- checked$keywords
  check_zero <- checked$all_pruned_topic_idx

  if (length(check_zero) == length(keywords)) {
    cli::cli_abort("Nothing left in `keywords`!")
  }
  return(keywords_checked[setdiff(seq_along(keywords), check_zero)])
}
