#' Read texts
#'
#' This function reads texts and creates a keyATM_docs object, which is a list of texts. The currently available sources are: 
#' \itemize{
#'    \item a quanteda dfm object (\strong{recommended})
#'    \item a data.frame (or a \pkg{tibble} tbl_df) with texts contained in a column named \code{text}
#'    \item a vector of file paths to text source files
#'    }
#' 
#'
#'
#' @param texts Input. keyATM_read takes dfm, data.frame, \pkg{tibble} tbl_df, or a vector of file paths.
#' @param encoding A character. Only used when \code{texts} is a vector of file paths. Default is \code{UTF-8}. 
#' @param check logical; If \code{TRUE}, check whether there are any problems in the structure of texts (e.g. extra spaces between texts). Default is \code{TRUE}.
#'
#' @return A keyATM_docs-class object. A list whose elements are splitted texts. The length of the list equals to the number of documents.
#'
#' @examples
#' \dontrun{
...
#'
#' @param docs A keyATM_docs-class object, generated by \code{keyATM_read()} function
#' @param keywords A list of keywords. The length 
#' @param prune logical; If \code{TRUE}, prune keywords that do not appear in \code{docs}. Default is \code{TRUE}.
#' @param label_size Numeric value that specifies the size of the keyword labels appearing in the plot. Default is 3.2.
#'
#' @return A keyATM_viz-class object, which is a list containing: \describe{
#'    \item{figure}{a ggplot2 object}
#'    \item{values}{a tibble object that stores values used to create the plot}
...
#'   
#'  # Keywords are in a list with its length equals to the number of keyword topics
#'  keywords <- list(
#'                    c("education", "child", "student"),  # Education
#'                    c("public", "health", "program"),  # Health
#'                  )
#'
#'  # Create an object to visualize keywords
...
#' 
#'  # Save a figure 
#'  filename = "visualize_keywords.pdf"
...
visualize_keywords <- function(docs, keywords, prune = TRUE, label_size = 3.2)
{
  ...
  # Check keywords and prune those do not appear in the corpus
