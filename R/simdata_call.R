...
#' @param alpha parameter of dirichlet of theta
#' @param beta_r parameter of dirichlet of phi_regular
#' @param beta_s parameter of dirichlet of phi_seed
#' @param gamma1 parameter of beta (set either gamma or p)
#' @param gamma2 parameter of beta (set either gamma or p)
#' @param p probability of using seed (set either gamma or p)
#' @param lambda parameter of poisson
#' @param seeds_len number of seeds words per each topic
#' @param prop_seeds proportion of seed words
#' @examples
#' \dontrun{create_sim_data(saveDir="/Users/Shusei/Desktop/temp/SimulationData/", D=200, K=10, TotalV=8000, alpha=0.1, beta_r=0.1, beta_s=0.1, gamma1=1, gamma2=20, lambda=300, seeds_len=5)}
#' \dontrun{create_sim_data(saveDir="/Users/Shusei/Desktop/temp/SimulationData/", D=200, K=10, TotalV=8000, alpha=0.1, beta_r=0.1, beta_s=0.1, p=0.3, lambda=300, seeds_len=5)}
#'
#' @export
create_sim_data <- function(saveDir,
                            D = 200,
                            K = 10,
                            TotalV = 8000,
                            alpha = 0.1,
  ...
  if (!is.null(p) && (!is.null(gamma1) | !is.null(gamma2)))
    stop("You cannnot set p and gamma at the same time.")

  if (is.vector(p)) {
    # specify the each topic's proportion, length should be equal to the number of topics
    if (length(p) != K) {
      stop(
        "p: p is given as vector. Length of p does not match with the number of topic. You need to specify each topic."
      )
    }
  ...
  # TotalV <- 8000
  V <-
    as.integer(TotalV / K) # the number of unique vocabulary for each topic
  ...
  for (i in 1:D) {
    ...
    ## Generate W, Z, and X
    wzx_list <-
      Gen_wzx(i, tmp_nd, phiR, phiS, p_use_seed, theta, V, seed_words)
  ...
  doc_list %>% purrr::map_df(`[`, c("w", "z")) %>%
    dplyr::group_by(w, z) %>%
    dplyr::summarize(count = n()) %>%
    tidyr::spread(key = w,  value = count) -> TopicVoc

  ## document_vocabulary assignment
  doc_list %>% purrr::map_df(`[`, c("w", "doc_id")) %>%
    dplyr::group_by(w, doc_id) %>%
    dplyr::summarize(count = n()) %>%
    tidyr::spread(key = w,  value = count) -> DocVoc


  ## document_topic assignment
  doc_list %>% purrr::map_df(`[`, c("doc_id", "z")) %>%
    dplyr::group_by(doc_id, z) %>%
    dplyr::summarize(count = n()) %>%
