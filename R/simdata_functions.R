...
## helper functions from MCMCPack and Laplace's Demon
## so we don't need those packages
...
Gen_seedwords <- function(K, seeds_len, V) {
  ...
  for (k in 1:K) {
    seed_words[k,] <-
      paste0(as.character(k), "S", as.character(sample(1:V, seeds_len)))
  }
  return(seed_words)
}
...
Gen_w_seeds <- function(index, phiR, phiS, z, x, seed_words) {
  ...
  if (indicator == 0) {
    ...
    word <-
      paste0(as.character(topic), "R", as.character(word)) # no overlap of words between topics
  } else {
    # Seed words
...
#' Save index freq
#'
#' @param doc_list document list
#' @param show_num how many to show
#' @param saveDir save directory
#'
#' @return
#' @export
#' @importFrom dplyr group_by ungroup summarize filter arrange select
...
Save_Index_Freq <- function(doc_list, show_num, saveDir) {
  ...
  write.csv(
    TopicVocFreq,
    paste0(saveDir, "topic_word_freq.csv"),
    na = "0",
    row.names = FALSE
  )
}
...
save_seed_words <- function(seed_words, p, saveDir) {
  if (!is.vector(p)) {
    write.csv(
      data.frame(seed_words),
      paste0(saveDir, "SeedWords.csv"),
      na = "0",
      row.names = FALSE
    )
  } else{
    write.csv(
      data.frame(seed_words)[p != 0,],
      paste0(saveDir, "SeedWords.csv"),
      na = "0",
      row.names = FALSE
    )
...
Save_info <-
  function(topic_voc,
    ...
    write.csv(
      topic_voc,
      paste0(saveDir, "topic_word.csv"),
      na = "0",
      row.names = FALSE
    )
    write.csv(doc_voc,
              paste0(saveDir, "doc_voc.csv"),
              na = "0",
              row.names = FALSE)
    write.csv(doc_topic,
              paste0(saveDir, "doc_topic.csv"),
              na = "0",
              row.names = FALSE)
...
Save_seeds_len <- function(doc_list, saveDir) {
  doc_list %>% map_df(`[`, c("z", "w", "x")) %>%
    ...
    spread(key = w,  value = count) -> tmp

  write.csv(
    tmp,
    paste0(saveDir, "seed_assignment_truth.csv"),
    na = "0",
    row.names = FALSE
  )
}
...
Match_wz <- function(w, z) {
  match <-
    mapply(intersect, w, z) # find the match of topic, word and X
...
Match_wzx <- function(w, z, x) {
  match <-
    mapply(intersect3, w, z, x) # find the match of topic, word and X
  match <- length(unlist(match))
  return(match)
}
