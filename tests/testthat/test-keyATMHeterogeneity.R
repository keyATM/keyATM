...
vars %>%
  tibble::as_tibble() %>%
  dplyr::mutate(
    Period = dplyr::case_when(Year <= 1899 ~ "18_19c", TRUE ~ "20_21c")
  ) %>%
  dplyr::mutate(
...
vars_selected %>%
  dplyr::mutate(
    Party = factor(Party, levels = c("Other", "Republican", "Democratic")),
    Period = factor(Period, levels = c("18_19c", "20_21c"))
  ) -> vars_selected
...
test_that("Covariates get", {
  ...
  expect_equivalent(
    colnames(mat),
    c("(Intercept)", "PartyRepublican", "PartyDemocratic", "Period20_21c")
  )
})
...
test_that("Doc Topic", {
  ...
  skip_on_os("linux")
  expect_equal(
    summary(strata_topic)[[2]]$Point[1],
    0.08544458,
    tolerance = 0.000001
  )
  expect_equal(
    summary(strata_topic)[[2]]$Upper[2],
    0.136887,
    tolerance = 0.000001
  )
  expect_equal(
    summary(strata_topic)[[2]]$Lower[3],
    0.2036373,
    tolerance = 0.000001
  )
})
