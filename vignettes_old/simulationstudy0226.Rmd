---
title: "Topic Number Misspecification"
...
explore_ <- explore(docs,
             ...
             remove_symbols = TRUE,
             remove_separators = TRUE)
explore_$words_distribution(1:30)
explore_$top_words(10)
explore_$visualize_words_documents("w65t5")
explore_$visualize_words(c('w11t1','w64t1', 'w273t1'))
```

# Use Simulation Data 1 (more topic)
Use high to middle proportion words:
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
explore_$visualize_words(c('w0t1','w67t1', 'w223t2', "w536t2", "w1656t3",
                      "w541t2", "w6t3", "w2629t3", "w1668t3"))
```

```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
seed_list <- list(c("w0t1 w67t1"),
                 c("w223t2 w536t2 w541t2"),
                 c("w6t3 w2629t3 w1668t3 w1656t3"))
model <- create_model("Sim1", seed_list=seed_list, extra_k=8)
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(1,2,3,7,9,5,8,4,6,10,11)
```
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
get_lda_result("Sim1", iter=iter_num, k=11, topicvec=c(6,8,10,2,5,9,3,1,4,7,11))
```

# Use Simulation Data 2 (more and more topic)
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
seed_list <- list(c("w0t1 w67t1"),
                 c("w223t2 w536t2 w541t2"),
                 c("w6t3 w2629t3 w1668t3 w1656t3"))
model <- create_model("Sim1", seed_list=seed_list, extra_k=15)
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(1,4,2,9,3,13,14,5,7,8,11,6,12,15,17,10,16,18)
```
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
get_lda_result("Sim1", iter=iter_num, k=18, 
               topicvec=c(3,9,12,14,16,4,5,18,8,15,13,1,2,6,10,11,7,17))
```

# Use Simulation Data 3 (fewer I)
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
seed_list <- list(c("w0t1 w67t1"),
                 c("w223t2 w536t2 w541t2"),
                 c("w6t3 w2629t3 w1668t3 w1656t3"))
...
### Heatmap
If the number of topics is fewer than the truth, there are contaminated topics. Topic 4 is a regular topic.
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
diagnosis_topic_recovery_heatmap(post, 25, topicvec=new_order)
```

```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
get_lda_result("Sim1", iter=iter_num, k=4, topicvec=c(3,4,2,1))
```

# Use Simulation Data 4 (fewer II)
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
seed_list <- list(c("w0t1 w67t1"),
                 c("w223t2 w536t2 w541t2"),
                 c("w6t3 w2629t3 w1668t3 w1656t3"))
model <- create_model("Sim1", seed_list=seed_list, extra_k=2)
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
```
...
### Heatmap
If the number of topics is fewer than the truth, there is a contaminated topic. Topic 4 and 5 are regular topic.
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
diagnosis_topic_recovery_heatmap(post, 25, topicvec=new_order)
```

```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
get_lda_result("Sim1", iter=iter_num, k=5, topicvec=c(4,1,5,3,2))
```
...
explore_ <- explore(docs,
             ...
             remove_symbols = TRUE,
             remove_separators = TRUE)
# explore_$top_words(250:300)
explore_$visualize_words(c("w1394t1", "w27t1", "w291t1",
                      "w828t2", "w1571t2", "w650t2", 
                      "w24t3", "w1597t3", "w1564t3"))
```
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
seed_list <- list(c("w1394t1 w27t1 w291t1"),
                 c("w828t2 w1571t2 w650t2"),
                 c("w24t3 w1597t3 w1564t3"))
model <- create_model("Sim1", seed_list=seed_list, extra_k=7)
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(1,6,2,10,3,8,7,4,5,9)
```
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
get_lda_result("Sim1", iter=iter_num, k=10, topicvec=c(7,8,10,6,9,2,1,4,3,5))
```
...
explore_ <- explore(docs,
             ...
             remove_separators = TRUE)
explore_$top_words(10)
explore_$visualize_words(c("w1077t1", "w1077t1", "w971t1", 
                      "w1986t2", "w386t2", "w1258t2",
                      "w926t3", "w851t3", "w984t3" ))
```
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
seed_list <- list(c("w1077t1 w1077t1 w971t1"),
                 c("w1986t2 w386t2 w1258t2"),
                 c("w926t3 w851t3 w984t3"))
model <- create_model("Sim1", seed_list=seed_list, extra_k=7)
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(5,6,1,2,8,9,10,7,4,3)
```
...
### Heatmap
Seed (EstTopic 1, 2 and 3) do not work.
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=6.5}
diagnosis_topic_recovery_heatmap(post, 25, topicvec=new_order)
```


# Low Proportion (More Seeds)
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
# Explore
folder <- paste0(data_folder, "Sim1", "/W")
docs <- list.files(folder, pattern = "*.txt", full.names = TRUE)
explore_ <- explore(docs,
             ...
             remove_separators = TRUE)
explore_$top_words(10)
explore_$visualize_words(c("w1077t1", "w1077t1", "w971t1", "w3448t1", "w2757t1",
                      "w1986t2", "w386t2", "w1258t2", "w1986t2", "w1552t2",
                      "w926t3", "w851t3", "w984t3", "w2433t3", "w1038t3"))
```
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
seed_list <- list(c("w1077t1 w1077t1 w971t1 w3448t1 w2757t1"),
                 c("w1986t2 w386t2 w1258t2 w1986t2 w1552t2"),
                 c("w926t3 w851t3 w984t3 w2433t3 w1038t3"))
model <- create_model("Sim1", seed_list=seed_list, extra_k=7)
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(5,7,9,10,1,6,8,2,4,3)
```
...
explore_ <- explore(docs,
             ...
             remove_separators = TRUE)
explore_$top_words(10)
explore_$visualize_words(c("w1077t1", "w1077t1", "w971t1", "w249t1",
                      "w1986t2", "w386t2", "w1258t2", "w943t2",
                      "w926t3", "w851t3", "w984t3", "w263t3" ))
```
```{r, warning=FALSE, message=FALSE, fig.width=6, fig.height=4}
seed_list <- list(c("w1077t1 w1077t1 w971t1 w249t1"),
                 c("w1986t2 w386t2 w1258t2 w943t2"),
                 c("w926t3 w851t3 w984t3 w263t3"))
model <- create_model("Sim1", seed_list=seed_list, extra_k=7)
res <- topicdict_train(model, iter = iter_num)
post <- topicdict::posterior(res)
new_order <- c(1,4,2,3,6,5,9,10,7,8)
```
